    ...
    <div className="space-y-6">
        ...
        <Card className="border-0 shadow-sm ring-1 ring-gray-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Savings (RSD)</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${(depositTotal - withdrawalTotal) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {(depositTotal - withdrawalTotal).toLocaleString('en-US', { maximumFractionDigits: 0 })}
                </p>
              </div>
              <div className={`p-3 rounded-full ${(depositTotal - withdrawalTotal) >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                <DollarSign className={`h-6 w-6 ${(depositTotal - withdrawalTotal) >= 0 ? 'text-green-600' : 'text-red-600'}`} />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="border-0 shadow-sm ring-1 ring-gray-200">
        <CardHeader className="pb-4">
          <CardTitle className="text-lg font-semibold text-gray-900">Savings by Currency</CardTitle>
          <p className="text-sm text-gray-600">Current balances in each currency</p>
        </CardHeader>
        <CardContent className="pb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(totals).map(([currency, amount]) => (
              <div key={currency} className="p-4 bg-gray-50 rounded-xl">
                <div className="flex items-center justify-between">
                  <div>
